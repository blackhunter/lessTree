...
module.exports  = {
		...
		fs.readFile(path, 'utf-8', function(err, file){
			if(!err){
				less.parse(file, function(err, less){
					if(!err){
						var sheet = new self.sheet(path, less);
	...
	saveStyle: function(obj, cb){
		fs.writeFile((typeof obj == 'string'? obj : obj.path), this.toLess(obj.tree), cb);
	},
	toLess: function(tree){
		endIgnore = false;
		currTabs = '';
		if(tree instanceof this.sheet)
			tree = tree.tree;
